...
class TreeMultiSimilarityLoss(nn.Module):
    ...
    def forward(self, anchor_embed, all_samples_embed, all_samples_dist):
        ...
        pos_loss = 1/self.alpha*torch.log(1+torch.sum(all_samples_dist*torch.exp(-self.alpha*(cdist-self.base)), dim=1))
        neg_loss = 1/self.beta*torch.log(1+torch.sum((1-all_samples_dist)*torch.exp(self.beta*(self.base-cdist)), dim=1))
        return torch.mean(pos_loss + neg_loss)
