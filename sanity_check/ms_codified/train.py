    ...
    while True:
        ...
        for tree_batch in batch_iterator:
            if tree_batch is not None:
                neg_samples_ids = tree_batch[0].to(args.device)
                all_samples_cont_idx = tree_batch[1].to(args.device)
                loss = model.get_tree_loss(neg_samples_ids, all_samples_cont_idx)
