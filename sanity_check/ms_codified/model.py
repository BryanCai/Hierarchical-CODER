...
import torch
from loss import AMSoftmax, HierarchicalMultiSimilarityLoss, HierarchicalTreeLogLoss, HierarchicalTreeLoss
...
class UMLSPretrainedModel(nn.Module):
    ...
    def get_tree_loss(self, all_input_ids, all_samples_cont_idx):
        neg_samples_output = self.get_sentence_feature(all_input_ids)

        loss = self.ms_loss(neg_samples_output, all_samples_cont_idx)
        return loss
