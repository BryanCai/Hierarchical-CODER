    ...
    while True:
        ...
        for tree_batch in batch_iterator:
            if tree_batch is not None:
                ...
                all_samples_ids   = tree_batch[1].to(args.device)
                all_samples_dists = tree_batch[2].to(args.device)
                loss = model.get_tree_loss(anchor_ids, all_samples_ids, all_samples_dists)
