...
class TreeDataset(Dataset):
    def __init__(self, tree_dir, model_name_or_path, max_neg_samples=8, max_length=32, eval_data_path=None):
        ...
        for tree in self.trees:
            if tree == 'phecode':
                ...
                self.obj_list += [(i, tree) for i in self.trees[tree].text.keys()] * 2
                self.len += len(self.trees[tree]) * 2
            else:   # loinc
                self.obj_list += [(i, tree) for i in self.trees[tree].text.keys()]
                self.len += len(self.trees[tree])
                self.leaf = [i for i in self.trees[tree].text.keys() if i.find('LP')<0]
        temp = []
        for tree in self.trees:
    ...
    def __getitem__(self, index):
        ...
        if tree == 'phecode':
            ...
            if anchor_id in self.leaf:
                neg_samples += [(i, 0) for i in self.trees[tree].children[anchor_id]]
                if anchor_id in self.trees[tree].parent:
                    parent = self.trees[tree].parent[anchor_id]
                    neg_samples += [(parent, 1)]
                    neg_samples += [(i, 0) for i in self.trees[tree].children[parent] if i != anchor_id]
                pos_samples = [(anchor_id, 0)] * self.max_neg_samples
            else:
