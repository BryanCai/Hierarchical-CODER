...
class TreeMultiSimilarityLoss(nn.Module):
    def __init__(self, alpha=2, beta=50, base=0.5, **kwargs):
        ...
        print('alpha: {}, beta: {}, base: {}'.format(alpha, beta, base))

    def forward(self, anchor_embed, all_samples_embed, all_samples_dist):
        # print(anchor_embed.shape)
        # print(all_samples_embed.shape)
        # print(all_samples_dist.shape)
        # anchor_embed = anchor_embed.reshape(-1, 24, 768)
        # all_samples_embed = all_samples_embed.reshape(-1, 24, 768)
        all_samples_dist = all_samples_dist.reshape(-1, 24)
        cdist = self.cos(anchor_embed, all_samples_embed).reshape(-1, 24)
        # print(all_samples_dist)
        # print(cdist)
        pos_loss = 1.0/self.alpha*torch.log(1+torch.sum(all_samples_dist*torch.exp(-self.alpha*(cdist-self.base)), dim=1))
        neg_loss = 1.0/self.beta*torch.log(1+torch.sum((1-all_samples_dist)*torch.exp(self.beta*(cdist-self.base)), dim=1))
        return torch.mean(pos_loss + neg_loss)
